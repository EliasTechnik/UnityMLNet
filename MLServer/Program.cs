...
namespace MLServer
{
    class Program
    {
            ...
            if(erg!=null){
                ...
                switch(instruction){
                    case "predict":
                        ...
                        break;
                     case "predict2":
                        Console.WriteLine("Im asking the AI2 about: {0}",xo.find("movement_data").Payload);
                        CSVLine l2=new CSVLine(xo.find("movement_data").Payload,',');
                        ModelInput2 data2=new ModelInput2(l2);
                        ModelOutput2 prediction2 = ai2.predict(data2);
                        XMLobject xp2=new XMLobject("ai_input");
                        Console.WriteLine("Answer: {0}",prediction2.Action);
                        //msg.Connection.addMessageToOutbox(new WSMessage(xp.serialize(),msg.Connection,true));
                        //server.sendText(msg.Connection,xp.serialize());
                        break;
        ...
        static void Main(string[] args)
        {
            ...
            //Console.WriteLine("Im going to train the ai!");
            ai2=new AIWrapper2();
            ai2.loadModel("model/ai2.zip");
            //Console.WriteLine("Saved Model to: {0}",ai2.trainNewModel("dataCollection/leveldat.csv","model/ai2.zip"));
