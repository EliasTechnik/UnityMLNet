...
public class AIWrapper{
    ...
    private static PredictionEngine<ModelInput2, ModelOutput2> _predEngine;
    private static ITransformer _trainedModel;
    private PredictionEngine<ModelInput, ModelOutput> predictionEngine;
    private IDataView trainingDataView; //for training
    ...
    private IEstimator<ITransformer> BuildAndTrainModel(IDataView trainingDataView, IEstimator<ITransformer> pipeline){
        ...
        _predEngine = mlContext2.Model.CreatePredictionEngine<ModelInput2, ModelOutput2>(_trainedModel);
        return trainingPipeline;
    }
    private void Evaluate(DataViewSchema trainingDataViewSchema,string _testPath){
        var testDataView = mlContext2.Data.LoadFromTextFile<ModelInput2>(_testPath,hasHeader: true);
        var testMetrics = mlContext2.MulticlassClassification.Evaluate(_trainedModel.Transform(testDataView));
    ...
    public AIWrapper(){
        ...
        mlContext2 = new MLContext();//Create MLContext2
    }
    public void loadModel(string path,int version){
        ...
        if(version==2){
            //predictionEngine = mlContext.Model.CreatePredictionEngine<ModelInput2, ModelOutput2>(predictionPipeline);
        }
    }
    ...
    public string trainNewModel(string name,string dataPath,string _testPath,string saveTo){
        mlContext2=new MLContext(0); //seed: 0
        trainingDataView=mlContext2.Data.LoadFromTextFile<ModelInput2>(dataPath,hasHeader: true);
        var pipeline=ProcessData();
        var trainingPipeline = BuildAndTrainModel(trainingDataView, pipeline);
        Evaluate(trainingDataView.Schema,_testPath); //nict unbedingt notwendig
        mlContext.Model.Save(_trainedModel, predictionPipelineSchema, saveTo);
